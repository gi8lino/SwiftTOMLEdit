//! Lossless TOML parsing and editing for Swift through a small C ABI.
...
fn parse_text(text: &str) -> ParseResponse {
...
unsafe fn input_string(input: *const c_char) -> Result<String, BridgeError> {
    ...
    // SAFETY: callers provide a NUL-terminated UTF-8 C string for this call.
...
fn apply_edit(document: &mut DocumentMut, edit: Edit) -> Result<(), BridgeError> {
    ...
    for component in tables {
        ...
        table = table
            ...
            .ok_or_else(|| bridge_error(format!("{component} is not a TOML table")))?;
    }
...
fn edit_text(text: &str, request_json: &str) -> Result<String, BridgeError> {
...
///
/// # Safety
/// `input` must point to a valid NUL-terminated UTF-8 string. The returned pointer
/// must be released with [`swift_toml_edit_string_free`].
pub unsafe extern "C" fn swift_toml_edit_parse(input: *const c_char) -> *mut c_char {
...
///
/// # Safety
/// Both arguments must point to valid NUL-terminated UTF-8 strings. The returned
/// pointer must be released with [`swift_toml_edit_string_free`].
pub unsafe extern "C" fn swift_toml_edit_edit(
    input: *const c_char,
...
/// already been freed.
pub unsafe extern "C" fn swift_toml_edit_string_free(value: *mut c_char) {
    if !value.is_null() {
...
mod tests {
    ...
    #[test]
    fn parse_returns_typed_root_table() {
...
"#,
        ...
        assert!(matches!(response.value, Some(BridgeValue::Table(_))));
    }
    ...
    #[test]
    fn edit_adds_missing_tables() {
        let request = r#"{"edits":[{"path":["app","ui","fields"],"value":{"kind":"string_array","value":["date","time"]}}]}"#;

        let output = edit_text("", request).expect("edit should succeed");
        let parsed = output
            .parse::<DocumentMut>()
            .expect("edited text should remain valid TOML");
        let fields = parsed["app"]["ui"]["fields"]
            .as_array()
