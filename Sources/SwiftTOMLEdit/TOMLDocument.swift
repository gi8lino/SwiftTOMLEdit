import CSwiftTOMLEdit
import Foundation

/// A TOML parsing or editing failure reported by the native `toml_edit` bridge.
public struct TOMLParseError: Error, LocalizedError, CustomStringConvertible, Sendable, Equatable {
...
/// A typed value parsed from a TOML document.
...
/// A TOML table parsed by the lossless Rust `toml_edit` engine.
public struct TOMLTable: Sendable, Equatable, Sequence {
  ...
  public init(string: String) throws {
    self = try TOMLDocument.parse(string)
  }

  public var isEmpty: Bool { values.isEmpty }
  public var count: Int { values.count }
  public var keys: Dictionary<String, TOMLValue>.Keys { values.keys }
  public var dictionary: [String: TOMLValue] { values }
...
/// One lossless TOML update applied at a key path.
public struct TOMLEdit: Sendable, Equatable {
...
/// Parses and losslessly edits TOML documents through Rust's `toml_edit` crate.
public enum TOMLDocument {
  /// Parses a TOML document into a typed root table.
  public static func parse(_ text: String) throws -> TOMLTable {
    let response = try NativeTOMLBridge.parse(text)
  ...
  /// Applies edits while preserving comments, whitespace, and unrelated ordering.
...
private enum NativeTOMLBridge {
  ...
  static func edit(_ text: String, edits: [TOMLEdit]) throws -> String {
    ...
    let response: [String: Any] = try text.withCString { input in
      try requestJSON.withCString { request in
        try decodeResponse(swift_toml_edit_edit(input, request))
      }
    }
  ...
  private static func decodeResponse(
    ...
    guard let pointer else { throw bridgeError("native TOML bridge returned no response") }
    defer { swift_toml_edit_string_free(pointer) }
    let data = Data(bytes: pointer, count: strlen(pointer))
    guard let response = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
      throw bridgeError("native TOML bridge returned an invalid response")
    }
    return response
  }
  ...
  private static func decodeValue(_ object: Any) throws -> TOMLValue {
    ...
    else { throw bridgeError("native TOML bridge returned an invalid value") }
    ...
    default:
      throw bridgeError("native TOML bridge returned unknown kind \(kind)")
    }
  }
